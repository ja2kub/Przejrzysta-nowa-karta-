...
def run():
    ...
    with sync_playwright() as p:
        ...
        try:
             page.click('#searchInput', force=True, timeout=1000)
             print('Clicked search input (force).')
        except:
             print('Could not click search input (expected due to pointer events?)')
        ...
        # 3. Check Menu Hidden (Manual Check)
        # Since hover throws error because of pointer-events interception (which is GOOD! means blocking works),
        # we can just assume hover won't trigger the listener on the child.
        # But let's verify logic:
        # Move mouse over where button is.
        controls = page.locator('#controlsRight')
        ...
        page.screenshot(path='verification/edit_mode_final_2.png')
        browser.close()
